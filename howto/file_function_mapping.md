...
| :--- | :--- |
| `/.env` | Runtime & build configuration file storing versioning parameters (`VERSION_CODE`, `VERSION_NAME`), target build output/log directories (`BUILD_OUTPUT_DIR`, `BUILD_LOGS_DIR`), target distribution build mode (`BUILD_TARGET` = `playstore`/`fdroid`/`both`), AdMob API parameters (`GOOGLE_ADS_API_KEY`, `INCLUDE_GOOGLE_ADS`), developer website & privacy URLs (`DEVELOPER_WEBSITE_URL`, `PRIVACY_POLICY_URL`), and GitHub repository URL (`GITHUB_REPO_URL`). |
...
| File / Path | Corresponding Function & Feature |
| :--- | :--- |
| **`MainActivity.kt`** | **Primary UI & User Interaction Layer (Jetpack Compose)**:<ul><li>**Top Header Bar**: Title display ("MILKYS APP"), dark/light theme switcher, settings modal trigger, and floating bubble toggle button.</li><li>**Master Boost Dial**: Power toggle button, gain percentage slider (+0% to +200%), and glowing ring status indicator.</li><li>**Sound Test Button**: Synthesizes and plays a 3-second multi-frequency melody (`playSoundTest3Sec`) directly next to the master power button.</li><li>**Hearing & Speaker Damage Warning Banner**: Prominent high-contrast alert card highlighting hearing loss and speaker damage risks (dismissable with 7-day remind timer).</li><li>**5-Band Equalizer Panel**: Frequency adjustment sliders (60Hz, 230Hz, 910Hz, 3.6kHz, 14kHz) with range from -15dB to +15dB.</li><li>**Preset Selector Pills**: Quick preset buttons (*Flat, Classical, Jazz, Pop, Rock, Custom*).</li><li>**Developer Support Notice Popup**: Interactive modal shown when toggling off ads explaining that the app is developed voluntarily and keeping ads enabled supports the developer.</li><li>**Developer Website Link**: Interactive setting item pointing to `DEVELOPER_WEBSITE_URL` that launches external web browser.</li><li>**Privacy Policy & Terms Page**: Dedicated modal displaying standard privacy terms, zero personal data usage disclosures, DSP audio permission details, and a clickable link to `PRIVACY_POLICY_URL`.</li><li>**Open Source License Page**: Dedicated modal displaying the full scrollable GNU General Public License v3.0 (GPLv3) text and a GitHub repository link button pointing to `GITHUB_REPO_URL`.</li><li>**Settings Modal**: Preferences for safety warnings, notification controls, developer website, privacy terms, open source license, onboarding replays, and a dropdown selector for 13 supported languages.</li><li>**Onboarding Screen**: Welcome dialog for first-time users.</li></ul> |
...
| `res/values-*/strings.xml` | Per-locale string resources for 13 supported languages (`values-id`, `values-in`, `values-ms`, `values-hi`, `values-pt`, `values-fr`, `values-it`, `values-de`, `values-zh-rCN`, `values-zh-rTW`, `values-ja`, `values-ko`). |
| `res/xml/locales_config.xml` | Android per-app language configuration manifest declaring all supported locales to the Android OS. |
